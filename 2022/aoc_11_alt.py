...
def run_rounds(nr_rounds, reduce_worry_level=True):

    # PART 2 setup and execution
    Monkeys = []
    for monkey in monkey_str:
        Monkeys.append(Monkey(monkey, Monkeys, reduce_worry_level=reduce_worry_level))

    if not reduce_worry_level:  # Find and set mult_div_nr to reduce items to lowest value with activity properties
        mult_div_nr = 1
    ...
    for round in range(nr_rounds):
        ...
        if debug:
            if reduce_worry_level:
                ...
                if (round + 1) % 100 == 0:
                    print(f'Round {round + 1}: {[monkey.activity for monkey in Monkeys]}')
    ...
    return max_activity * second_max_activity


print(f'Result of part 1: {run_rounds(20)}')
print(f'Result of part 2: {run_rounds(10000, False)}')
