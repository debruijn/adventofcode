...
fn get_dist(x: (isize, isize), mean_pos0: f32, mean_pos1: f32) -> f32 {
    ((x.0 as f32 - mean_pos0).powf(2.0) +
        (x.1 as f32 - mean_pos1).powf(2.0)).powf(0.5)
}


fn get_total_dist(t: &isize, nums: &Vec<[isize; 4]>) -> f32 {
    let pos_t = nums.iter().map(|x| (x[0] + x[2]*t, x[1] + x[3]*t));
    let mean_pos0: f32 = nums.iter().map(|x| x[0] + x[2]*t).sum::<isize>() as f32 / nums.len() as f32;
    let mean_pos1: f32 = nums.iter().map(|x| x[1] + x[3]*t).sum::<isize>() as f32 / nums.len() as f32;
    pos_t.map(|x| get_dist(x, mean_pos0, mean_pos1))
        .sum()
}


fn print_pos(t: &isize, nums: &Vec<[isize; 4]>) {
    let pos_t: Vec<(isize, isize)> = nums.iter().map(|x| (x[0] + x[2]*t, x[1] + x[3]*t)).collect();
    let lims0 = (nums.iter().map(|x| x[0] + x[2]*t).min().unwrap(),
                 nums.iter().map(|x| x[0] + x[2]*t).max().unwrap());
    let lims1 = (nums.iter().map(|x| x[1] + x[3]*t).min().unwrap(),
