...
def get_next_step(player, candidates, free, checked):
    ...
    new_candidates = tuple(set([x for loc in candidates for x in new(loc) if x in free and x not in checked]))
...
def run_all(example_run: Union[int, bool]):
                    ...
                    if target[2] <= 0:
                        ...
                        if sum(enemy[2]>0 for enemy in enemies) == 0:
                            ...
                            if player != [iter_player for iter_player in players if iter_player[2]>0][-1]:
                                round -= 1
                            break
            if any(elf[2] <= 0 for elf in elves):
                break
            players = [player for player in players if player[2] > 0]
            round += 1

            if debug:
                print(f"Round {round}: {len(players)} left: {sum(x[2] for x in players)}/{sum(x[2] for x in goblins)}/"
                      f"{sum(x[2] for x in elves)} -- {players}")
        if all(elf[2] > 0 for elf in elves):
            attack_stop = True

    result_part2 = sum(player[2] for player in players) * round
    ...
    extra_out = {'Initial team size': f"{len(goblins)} goblins vs {len(elves)} elves",
                 ...
                 'Attack of elves needed': attack,
                 'Rounds needed for battle in part 2': round}
