...
from aoc_rust import find_recipe

use_rust = True  # Ignore Python settings and run in Rust in naive way. Timings: 2.8s (Rust) vs 19.4s (Python with acc)

# Python settings and explanation below
use_strings = False
...
def run_all(example_run: Union[int, bool]):
    ...
    if use_rust:
        result_part1, result_part2 = find_recipe(data[0])
        result_part1 = "".join(str(x) for x in result_part1)
        extra_out = {'Number of digits to skip': data[0]}
    else:
